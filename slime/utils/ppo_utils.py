...
def compute_sao_dis_loss(
    ...
    """Compute the SAO direct double-sided importance-sampling surrogate.

    SAO compares the current policy directly with the rollout behavior policy
    and completely removes tokens whose ratio is outside the open interval
    ``(1 - eps_low, 1 + eps_high)``. In particular, this is *not* PPO's
    advantage-dependent directional clipping: both tails are removed for both
    advantage signs. The open interval follows Eq. (4) of arXiv:2607.07508;
    values exactly on either boundary are removed.

    The returned loss is the conventional direct-ratio policy-gradient
    surrogate ``-ratio * advantage`` on retained response tokens. The
    behavior log-probabilities are inputs recorded by the rollout engine; no
    old-policy probabilities are accepted by this primitive.

    Returns per-token ``(loss, below, above, retained_ratio)`` tensors.
    ``below`` and ``above`` include exact-boundary tokens and exclude
    non-response tokens; ``retained_ratio`` is zero for every removed token so
    extreme stale-policy ratios cannot overflow merely for monitoring. The
    caller owns response-token reduction.
    """
    if behavior_log_probs.shape != policy_log_probs.shape:
        raise ValueError(
            "behavior_log_probs and policy_log_probs must have identical shapes, "
            f"got {behavior_log_probs.shape} and {policy_log_probs.shape}"
        )
    if advantages.shape != policy_log_probs.shape or response_mask.shape != policy_log_probs.shape:
        raise ValueError("advantages and response_mask must match the policy_log_probs shape")
    if not math.isfinite(eps_low) or not 0.0 < eps_low < 1.0:
        raise ValueError(f"eps_low must be in (0, 1), got {eps_low}")
    if not math.isfinite(eps_high) or not eps_high > 0.0:
        raise ValueError(f"eps_high must be greater than zero, got {eps_high}")

    response_mask = response_mask.bool()
    log_ratio = policy_log_probs - behavior_log_probs
    if torch.any(response_mask & ~torch.isfinite(log_ratio)):
        raise FloatingPointError("SAO DIS received a non-finite current/behavior log-probability ratio")

    # Comparing in log space avoids overflow for badly stale trajectories. A
    # ratio is exponentiated only after the strict trust-region decision.
    lower_log_ratio = torch.log1p(policy_log_probs.new_tensor(-eps_low))
    upper_log_ratio = torch.log1p(policy_log_probs.new_tensor(eps_high))
    below = response_mask & (log_ratio <= lower_log_ratio)
    above = response_mask & (log_ratio >= upper_log_ratio)
    keep = response_mask & (log_ratio > lower_log_ratio) & (log_ratio < upper_log_ratio)

    safe_log_ratio = torch.where(keep, log_ratio, torch.zeros_like(log_ratio))
    retained_ratio = torch.exp(safe_log_ratio) * keep.to(policy_log_probs.dtype)
    loss = -retained_ratio * advantages
    return loss, below.to(policy_log_probs.dtype), above.to(policy_log_probs.dtype), retained_ratio
...
def get_skip_observation_advantages_and_returns(
    ...
    """Compute token GAE over model-action indices while skipping observations.

    ``action_mask`` identifies tokens generated by the policy. GAE recursion
    runs only over those indices, so the last token of one action bridges
    directly to the first token of the next action. Values at intervening
    environment-observation tokens cannot affect either the policy advantage
    or critic target.

    Policy advantages use ``policy_lambd`` while value targets use a separate
    ``critic_lambd``. This supports SAO's length-adaptive policy lambda and
    ``lambda_critic = 1`` without silently training the critic against the
    policy estimator. Observation positions are returned as zero and remain
    excluded by the caller's loss mask.
    """
    if values.ndim != 1 or rewards.ndim != 1 or action_mask.ndim != 1:
        raise ValueError("values, rewards, and action_mask must be one-dimensional")
    if values.shape != rewards.shape or values.shape != action_mask.shape:
        raise ValueError("values, rewards, and action_mask must have identical shapes")
    if not 0.0 <= gamma <= 1.0:
        raise ValueError(f"gamma must be in [0, 1], got {gamma}")
    if not 0.0 <= policy_lambd <= 1.0:
        raise ValueError(f"policy_lambd must be in [0, 1], got {policy_lambd}")
    if not 0.0 <= critic_lambd <= 1.0:
        raise ValueError(f"critic_lambd must be in [0, 1], got {critic_lambd}")

    action_indices = torch.nonzero(action_mask.bool(), as_tuple=False).flatten()
    if action_indices.numel() == 0:
        raise ValueError("skip-observation GAE requires at least one generated action token")

    # Compacting to action indices makes adjacent elements in these tensors
    # the action-to-action bridge from Eq. (6)-(7), regardless of how many
    # observation tokens lie between them. Reuse the chunked prefix scan so
    # 128k-token trajectories do not degrade into one GPU kernel per token.
    action_values = values[action_indices].unsqueeze(0)
    action_rewards = rewards[action_indices].unsqueeze(0)
    policy_action_advantages, _ = chunked_gae(action_rewards, action_values, gamma, policy_lambd)
    critic_action_advantages, _ = chunked_gae(action_rewards, action_values, gamma, critic_lambd)

    policy_advantages = torch.zeros_like(values)
    critic_returns = torch.zeros_like(values)
    policy_advantages[action_indices] = policy_action_advantages.squeeze(0)
    critic_returns[action_indices] = critic_action_advantages.squeeze(0) + action_values.squeeze(0)

    return policy_advantages.detach(), critic_returns.detach()


@torch.no_grad()
def get_skip_observation_advantages_and_returns_batch(
    total_lengths: list[int],
    response_lengths: list[int],
    values_list: list[torch.Tensor],
    token_rewards_list: list[torch.Tensor],
    sequence_rewards: list[float] | torch.Tensor,
    action_masks: list[torch.Tensor],
    gamma: float,
    length_adaptive_alpha: float,
    critic_lambd: float = 1.0,
) -> tuple[list[torch.Tensor], list[torch.Tensor]]:
    """Batch/CP wrapper for action-index GAE with raw-length policy lambda.

    The terminal sequence reward is assigned to the final generated action
    token, never to a trailing observation token. ``l`` in
    ``1 - 1 / (alpha * l)`` is the raw response-token length, matching the
    token-level SAO setup rather than the number of action tokens.
    """
    from megatron.core import mpu

    batch_size = len(response_lengths)
    fields = (total_lengths, values_list, token_rewards_list, sequence_rewards, action_masks)
    if any(len(field) != batch_size for field in fields):
        raise ValueError("all skip-observation GAE batch fields must have the same length")
    if batch_size == 0:
        return [], []
    ...
    for total_len, response_len, values, token_rewards, sequence_reward, action_mask in zip(
        total_lengths,
        response_lengths,
        values_list,
        token_rewards_list,
        ...
        if action_mask.numel() != response_len:
            raise ValueError(f"action mask length {action_mask.numel()} does not match response length {response_len}")

        if cp_size > 1:
            from slime.backends.megatron_utils.cp_utils import all_gather_with_cp

            full_values = all_gather_with_cp(values, total_len, response_len)
            full_token_rewards = all_gather_with_cp(token_rewards, total_len, response_len)
        else:
            full_values = values
            full_token_rewards = token_rewards

        full_values = full_values[:response_len]
        full_rewards = full_token_rewards[:response_len].clone()
        full_action_mask = action_mask.to(device=full_values.device, dtype=torch.bool)
        action_indices = torch.nonzero(full_action_mask, as_tuple=False).flatten()
        if action_indices.numel() == 0:
            raise ValueError("skip-observation GAE requires at least one generated action token")
        full_rewards[action_indices[-1]] += torch.as_tensor(
            sequence_reward,
            dtype=full_rewards.dtype,
            device=full_rewards.device,
        )

        policy_lambd = 1.0 - 1.0 / (length_adaptive_alpha * response_len)
        if not 0.0 <= policy_lambd <= 1.0:
            raise ValueError(
                "length-adaptive policy lambda fell outside [0, 1]: "
                f"alpha={length_adaptive_alpha}, response_len={response_len}, lambda={policy_lambd}"
            )
        full_advantages, full_returns = get_skip_observation_advantages_and_returns(
            full_values,
            full_rewards,
            full_action_mask,
            gamma,
        ...
        if cp_size > 1:
            ...
            full_advantages = slice_log_prob_with_cp(full_advantages, total_len, response_len)
