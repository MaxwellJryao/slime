...
@pytest.mark.unit
def test_masked_mean_excludes_masked_nonfinite_values_before_arithmetic(
    no_op_all_reduce,
) -> None:
...
def test_critic_metrics_are_microbatch_partition_invariant(
    chunks,
    no_op_all_reduce,
) -> None:
...
@pytest.mark.unit
def test_explained_variance_uses_global_moments_not_mean_of_local_ev(
    no_op_all_reduce,
) -> None:
    predictions = torch.tensor([0.0, 2.0, 10.0, 10.0])
    targets = torch.tensor([0.0, 1.0, 10.0, 20.0])
